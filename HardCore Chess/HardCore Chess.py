...
class Player(object):
    ...
    def getmove(self, board):
        global firstClick,secondClick,globalself,globalboard
        globalself = self
        globalboard = board
        #print("Getting move")
...
class Game(object):
    ...
    def run(self, player): # qwerty
        global globalgameself,globalplayera,globalplayerb,cplayer,titletext
        globalgameself = self
...
def drawPiece(piece,gridPos,canvas):
    ...
    for i in range(len(imagelist)):
        if imagelist[i] == piece:
            imagepos = i+1
    canvas.create_image((gridPos[1]*64)+32,((7-gridPos[0])*64)+32,image=imagelist[imagepos])

def loadpieces():
    l = []
    for i in (["R","KN","B","Q","K","WP","p","r","kn","b","q","k"]):
        if i == i.lower():
            i = "B" + i.upper()
...
def update():
    global firstClick,secondClick,start,globalplayerself,cplayer,titletext,timesincestart,turntime,inCheck,globalself,mycanvas2,checkMate
...
checkMate = False
mywindow = tk.Tk()
mywindow.title("Natty's Hardcore Chess")
mywindow.grid()
