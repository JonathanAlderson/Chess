...
class Game(object):
    ...
    def run(self, player): # qwerty
        global globalgameself,globalplayera,globalplayerb,cplayer,titletext,winner,invalidMove,firstClick,secondClick
        globalgameself = self
        self.refreshscreen(player)
        while True:
            try:
                global turnstarttime,player1Time,player2Time   # send help
                if player1AI == True and str(cplayer)[7] == "1":
                     print("Player AI is True")
                     start, target = playerAIMove(self.board)
                     firstClick = start
                     secondClick = target
